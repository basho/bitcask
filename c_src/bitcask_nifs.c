...
ERL_NIF_TERM bitcask_nifs_keydir_new1(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if (enif_get_string(env, argv[0], name, sizeof(name), ERL_NIF_LATIN1))
    {
        ...
        else
        {
            ...
            keydir = enif_alloc(sizeof(bitcask_keydir) + name_sz + 1);
...
ASYNC_NIF_DECL(
    ...
    { // pre
        ...
        args->data = enif_make_copy(ASYNC_NIF_WORK_ENV, argv[1]);
        enif_keep_resource((void*)args->handle);
    },
    { // work

        ErlNifBinary data;
        if (!enif_inspect_binary(env, args->data, &data)) {
        ...
        if (args->handle->is_write_lock)
        {
            ...
            if (pwrite(args->handle->fd, data.data, data.size, 0) == -1)
            {
...
ASYNC_NIF_DECL(
    ...
    { // pre
      ...
      args->flags = get_file_open_flags(env, argv[1]);
    },
    { // work
...
ASYNC_NIF_DECL(
    ...
    { // pre
        ...
        if (!(enif_get_resource(env, argv[0], bitcask_file_RESOURCE, (void**)&(args->handle)) &&
              enif_get_ulong(env, argv[1], &(args->offset_ul)) && /* Offset */
              enif_is_list(env, argv[2]))) {  /* Bytes to write */
          ASYNC_NIF_RETURN_BADARG();
        }
        args->data = enif_make_copy(ASYNC_NIF_WORK_ENV, argv[2]);
        enif_keep_resource((void*)args->handle);
    },
    { // work

        ErlNifBinary data;
        if (!enif_inspect_iolist_as_binary(env, args->data, &data)) {
          ASYNC_NIF_REPLY(enif_make_badarg(env));
          return;
        }
        ...
        while (count > 0)
        {
            bytes_written = pwrite(args->handle->fd, buf, count, args->offset_ul);
            if (bytes_written > 0)
            {
                count -= bytes_written;
                args->offset_ul += bytes_written;
                buf += bytes_written;
            }
            else
            {
...
#endif
    {"keydir_new_nif", 0, bitcask_nifs_keydir_new0},
    {"keydir_new_nif", 1, bitcask_nifs_keydir_new1},
    {"keydir_mark_ready_nif", 1, bitcask_nifs_keydir_mark_ready},
    {"keydir_put_nif", 9, bitcask_nifs_keydir_put},
    {"keydir_get_nif", 2, bitcask_nifs_keydir_get},
    {"keydir_remove_nif", 2, bitcask_nifs_keydir_remove},
    {"keydir_remove_nif", 5, bitcask_nifs_keydir_remove},
    {"keydir_copy_nif", 1, bitcask_nifs_keydir_copy},
    {"keydir_itr_nif", 4, bitcask_nifs_keydir_itr},
    {"keydir_itr_next_nif", 1, bitcask_nifs_keydir_itr_next},
    {"keydir_itr_release_nif", 1, bitcask_nifs_keydir_itr_release},
    {"keydir_info_nif", 1, bitcask_nifs_keydir_info},
    {"keydir_release_nif", 1, bitcask_nifs_keydir_release},
