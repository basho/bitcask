    ...
    Args1 = 
        case length(Args0) of
            3 ->
                ...
                Files = lists:usort(Files0), 
                [Dirname, Opts, Files];
            _ ->
                %% whole directory merges don't need to be sorted
                Args0
        end,
...
merge_items(New, Old) ->
    ...
    case {size(New), size(Old)} of
        {3, 3} ->
            OFiles = element(3, Old),
            NFiles = lists:usort(element(3, New)),
            {Dirname, Opts, lists:umerge(NFiles, OFiles)};
        {2, 2} ->
            {Dirname, Opts};
        {2, 3} ->
